...
/* ──────────────────────────────────────────────────────────────────────
   ADD / EDIT (expense / income form)
   ────────────────────────────────────────────────────────────────────── */
function AddForm({ kind, editTxn, onClose, onSave }) {
  ...
  // Cat / subcat / acc options from catalog
  const incomeCatNames = ['Salary', 'Freelance', 'Other', 'Gift', 'Refund'];
  const expenseCatNames = APP_DATA.categories.map((c) => c.name);
  const cats = isIncome ? incomeCatNames : expenseCatNames;
  // Subcategories scope to the chosen expense category.
  const selectedCatObj = APP_DATA.categories.find(c => c.name === cat);
  const subs = (!isIncome && selectedCatObj)
    ? (APP_DATA.subcategories || []).filter(s => s.categoryId === selectedCatObj.id).map(s => s.name)
    : [];
  const accs = APP_DATA.accounts.map((a) => a.name);

  // Reset sub when category changes — skip first render to preserve edit pre-fill
  const isFirstRender = useRefS(true);
  useEffectS(() => {
  ...
  function handleSave() {
    onSave({
      ...
      name: name.trim() || (isIncome ? 'New income' : 'New expense'),
      category: cat || (isIncome ? 'Salary' : 'Food'),
      sub: sub || null,
      account: acc || 'HDFC',
  ...
  // Date/time state — initialized from editTxn or now
  const nowIso = useMemoS(() => {
    const d = isEditing ? new Date(editTxn.date) : new Date();
  ...
  return (
    <div className="form-screen screen-enter" style={{ background: 'rgba(6,6,10,0.55)', backdropFilter: 'blur(30px)', WebkitBackdropFilter: 'blur(30px)' }}>
      ...
      <div className="form-body">
        ...
        <SearchableChips
          ...
          iconFor={(c) => {
            ...
            const hit = (APP_DATA.categories || []).find(x => x.name === c);
        ...
        {!isIncome && (
          <SearchableChips
            ...
            iconFor={(s) => {
              const hit = (APP_DATA.subcategories || []).find(x => x.name === s);
              return hit ? hit.icon : null;
            }}
          />
        )}
        ...
        <SearchableChips
          ...
          iconFor={(a) => {
            const hit = (APP_DATA.accounts || []).find(x => x.name === a);
            return hit ? hit.icon : null;
          }}
        />
      </div>
      ...
      <div className="save-bar">
        <button className={`save-btn ${isIncome ? 'income' : ''}`} onClick={handleSave}>
          {isEditing ? 'Update' : 'Save'} {isIncome ? 'income' : 'expense'}{amount ? ` · ₹${amount}` : ''}
        </button>
      </div>
    </div>);
