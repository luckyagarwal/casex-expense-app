...
function mapCatalogD(boot) {
  const cats = (boot.categories||[]).map(c=>({id:c.id,name:c.name,icon:resolveIconD(c.icon,'📁')}));
  const subs = (boot.subcategories||[]).map(s=>({id:s.id,name:s.name,icon:resolveIconD(s.icon,'🏷️')}));
  const accs = (boot.accounts||[]).map(a=>({id:a.id,name:a.name,icon:resolveIconD(a.icon,'🏦'),tail:null}));
  const r = boot.recent||{};
